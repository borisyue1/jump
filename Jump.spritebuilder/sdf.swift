...
//  Jump
//
//  Created by Boris Yue on 7/29/15.
...
class GameCenterInteractor: NSObject {
    ...
    */
    ...
    func reportHighScoreToGameCenterEasy(score: Int) {
        var scoreReporter = GKScore(leaderboardIdentifier: "BounceyBlobLeaderboard")
    ...
    func reportHighScoreToGameCenterHard(score: Int) {
        var scoreReporter = GKScore(leaderboardIdentifier: "BounceyBlobLeaderboardHardcore")
        scoreReporter.value = Int64(score)
        var scoreArray: [GKScore] = [scoreReporter]
        GKScore.reportScores(scoreArray, withCompletionHandler: {(error : NSError!) -> Void in
            if error != nil {
